...
public class Validator {
	...
	Validator(String id, String password){
		String hash = getHash(id);
		try {
			this.isValid = validatePassword(password, hash);
	...
	public String getHash(String id) {
		String dbhash = " ";
		String selectSql = "SELECT hash FROM grizzlydb.hash WHERE id = '" + id + "'";
		try {
			stmt = connection.createStatement();
			result = stmt.executeQuery(selectSql);
			result.next();
			dbhash = result.getString("hash");
